...
function activate(context) {
    ...
    bookmarkDecorationType.length = Bookmark_1.MAX_BOOKMARKS;
    for (let index = 0; index < Bookmark_1.MAX_BOOKMARKS; index++) {
        ...
        if (activeEditor.document.lineCount === 1 && activeEditor.document.lineAt(0).text === "") {
            ...
            if (invalids.length > 0) {
                ...
                for (let indexI = 0; indexI < invalids.length; indexI++) {
                    activeBookmark.bookmarks[invalids[indexI]] = Bookmark_1.NO_BOOKMARK_DEFINED;
                }
            }
        }
    }
            ...
            if (navigateThroughAllFiles === "replace") {
                ...
                for (let element of bookmarks.bookmarks) {
                    if (element.fsPath !== activeBookmark.fsPath) {
                        element.bookmarks[n] = Bookmark_1.NO_BOOKMARK_DEFINED;
                    }
                }
            }
        }
    ...
    function clearBookmark(n) {
        activeBookmark.bookmarks[n] = Bookmark_1.NO_BOOKMARK_DEFINED;
    }
        ...
        switch (navigateThroughAllFiles) {
            case "replace":
                ...
                if (activeBookmark.bookmarks[n] < 0) {
                    ...
                    //     let element = bookmarks.bookmarks[ index ];
                    for (let element of bookmarks.bookmarks) {
                        if ((element.fsPath !== activeBookmark.fsPath) && (element.bookmarks[n] !== Bookmark_1.NO_BOOKMARK_DEFINED)) {
                            // open and novigate
            ...
            case "allowDuplicates":
                ...
                if ((activeBookmark.bookmarks[n] > Bookmark_1.NO_BOOKMARK_DEFINED) &&
                ...
                for (let index = currentFile; index < bookmarks.bookmarks.length; index++) {
                    let element = bookmarks.bookmarks[index];
                    if ((!found) && (element.fsPath !== activeBookmark.fsPath) && (element.bookmarks[n] !== Bookmark_1.NO_BOOKMARK_DEFINED)) {
                        found = true;
                        // open and novigate
                        let uriDocument = vscode.Uri.file(element.fsPath);
                        vscode.workspace.openTextDocument(uriDocument).then(doc => {
