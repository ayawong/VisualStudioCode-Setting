...
class Sticky {
    static stickyBookmarks(event, activeEditorCountLine, activeBookmark, activeEditor) {
                ...
                if (event.document.lineCount > activeEditorCountLine) {
                    ...
                    if (event.contentChanges[0].range.end.line - event.contentChanges[0].range.start.line === 1) {
                        if ((event.contentChanges[0].range.end.character === 0) &&
                            ...
                            if (idxbk > -1) {
                                activeBookmark.bookmarks[idxbk] = exports.NO_BOOKMARK_DEFINED;
                            }
                        }
                    }
                    if (event.contentChanges[0].range.end.line - event.contentChanges[0].range.start.line > 1) {
                        for (let i = event.contentChanges[0].range.start.line /* + 1*/; i <= event.contentChanges[0].range.end.line; i++) {
                            ...
                            if (index > -1) {
                                activeBookmark.bookmarks[index] = exports.NO_BOOKMARK_DEFINED;
    ...
    static moveStickyBookmarks(direction, activeBookmark, activeEditor) {
        let diffChange = -1;
        let updatedBookmark = false;
        let diffLine;
