...
const BookmarksController_1 = require("./BookmarksController");
...
function activate(context) {
    ...
    let bookmarkDecorationType = [];
    bookmarkDecorationType.length = BookmarkFile_1.MAX_BOOKMARKS;
    for (let index = 0; index < BookmarkFile_1.MAX_BOOKMARKS; index++) {
        let pathIcon = context.asAbsolutePath("images\\bookmark" + index + ".png");
    ...
    vscode.workspace.onDidChangeTextDocument(event => {
        if (activeEditor && event.document === activeEditor.document) {
            ...
            if (activeBookmark && activeBookmark.bookmarks.length > 0) {
                ...
                updatedBookmark = Sticky_1.Sticky.stickyBookmarks(event, activeEditorCountLine, activeBookmark, activeEditor);
            }
        ...
        if (activeEditor.document.lineCount === 1 && activeEditor.document.lineAt(0).text === "") {
            ...
            if (invalids.length > 0) {
                ...
                for (let indexI = 0; indexI < invalids.length; indexI++) {
                    activeBookmark.bookmarks[invalids[indexI]] = BookmarkFile_1.NO_BOOKMARK_DEFINED;
                }
            }
        }
    }
    ...
    function revealLine(line, directJump) {
    ...
    function canSaveBookmarksInProject() {
        let saveBookmarksInProject = vscode.workspace.getConfiguration("numberedBookmarks").get("saveBookmarksInProject", false);
    ...
    function loadWorkspaceState() {
        ...
        bookmarks = new BookmarksController_1.BookmarksController();
        if (saveBookmarksInProject) {
            if (!vscode.workspace.workspaceFolders) {
                return false;
            }
            let bookmarksFileInProject = path.join(vscode.workspace.workspaceFolders[0].uri.fsPath, ".vscode", "numbered-bookmarks.json");
    ...
    function toggleBookmark(n, line) {
        ...
        if (index !== n) {
            ...
            let navigateThroughAllFiles = vscode.workspace.getConfiguration("numberedBookmarks").get("navigateThroughAllFiles", "false");
            if (navigateThroughAllFiles === "replace") {
                // for (let index = 0; index < bookmarks.bookmarks.length; index++) {
                //     let element = bookmarks.bookmarks[ index ];
                for (let element of bookmarks.bookmarks) {
                    if (element.fsPath !== activeBookmark.fsPath) {
                        element.bookmarks[n] = BookmarkFile_1.NO_BOOKMARK_DEFINED;
                    }
                }
            }
        }
    ...
    function clearBookmark(n) {
        activeBookmark.bookmarks[n] = BookmarkFile_1.NO_BOOKMARK_DEFINED;
    }
        ...
        switch (navigateThroughAllFiles) {
            case "replace":
                ...
                if (activeBookmark.bookmarks[n] < 0) {
                    ...
                    //     let element = bookmarks.bookmarks[ index ];
                    for (let element of bookmarks.bookmarks) {
                        if ((element.fsPath !== activeBookmark.fsPath) && (element.bookmarks[n] !== BookmarkFile_1.NO_BOOKMARK_DEFINED)) {
                            // open and novigate
                            let uriDocument = vscode.Uri.file(element.fsPath);
                            vscode.workspace.openTextDocument(uriDocument).then(doc => {
            ...
            case "allowDuplicates":
                ...
                if ((activeBookmark.bookmarks[n] > BookmarkFile_1.NO_BOOKMARK_DEFINED) &&
                ...
                let currentFile = bookmarks.indexFromUri(activeBookmark.fsPath);
                let found = false;
                // to the end
                for (let index = currentFile; index < bookmarks.bookmarks.length; index++) {
                    let element = bookmarks.bookmarks[index];
                    if ((!found) && (element.fsPath !== activeBookmark.fsPath) && (element.bookmarks[n] !== BookmarkFile_1.NO_BOOKMARK_DEFINED)) {
                        found = true;
                        // open and novigate
                        let uriDocument = vscode.Uri.file(element.fsPath);
                        vscode.workspace.openTextDocument(uriDocument).then(doc => {
